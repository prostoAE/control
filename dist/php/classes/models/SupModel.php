...
GROUP BY
  ...
  public function getDataFromSourceTable($startDate, $endDate) {
    ...
    if(src.super_ind = 1 and src.mag_001 > 0, an.billing_cost_per_service, max(src.mag_001)) as mag_001,
    if(src.super_ind = 1 and src.mag_003 > 0, an.billing_cost_per_service, max(src.mag_003)) as mag_003,
    if(src.super_ind = 1 and src.mag_007 > 0, an.billing_cost_per_service, max(src.mag_007)) as mag_007,
    if(src.super_ind = 1 and src.mag_009 > 0, an.billing_cost_per_service, max(src.mag_009)) as mag_009,
    if(src.super_ind = 1 and src.mag_010 > 0, an.billing_cost_per_service, max(src.mag_010)) as mag_010,
    if(src.super_ind = 1 and src.mag_011 > 0, an.billing_cost_per_service, max(src.mag_011)) as mag_011,
    if(src.super_ind = 1 and src.mag_012 > 0, an.billing_cost_per_service, max(src.mag_012)) as mag_012,
    if(src.super_ind = 1 and src.mag_014 > 0, an.billing_cost_per_service, max(src.mag_014)) as mag_014,
    if(src.super_ind = 1 and src.mag_015 > 0, an.billing_cost_per_service, max(src.mag_015)) as mag_015,
    if(src.super_ind = 1 and src.mag_016 > 0, an.billing_cost_per_service, max(src.mag_016)) as mag_016,
    if(src.super_ind = 1 and src.mag_018 > 0, an.billing_cost_per_service, max(src.mag_018)) as mag_018,
    if(src.super_ind = 1 and src.mag_020 > 0, an.billing_cost_per_service, max(src.mag_020)) as mag_020,
    if(src.super_ind = 1 and src.mag_022 > 0, an.billing_cost_per_service, max(src.mag_022)) as mag_022,
    if(src.super_ind = 1 and src.mag_023 > 0, an.billing_cost_per_service, max(src.mag_023)) as mag_023,
    if(src.super_ind = 1 and src.mag_024 > 0, an.billing_cost_per_service, max(src.mag_024)) as mag_024,
    if(src.super_ind = 1 and src.mag_025 > 0, an.billing_cost_per_service, max(src.mag_025)) as mag_025,
    if(src.super_ind = 1 and src.mag_026 > 0, an.billing_cost_per_service, max(src.mag_026)) as mag_026,
    if(src.super_ind = 1 and src.mag_027 > 0, an.billing_cost_per_service, max(src.mag_027)) as mag_027,
    if(src.super_ind = 1 and src.mag_028 > 0, an.billing_cost_per_service, max(src.mag_028)) as mag_028,
    if(src.super_ind = 1 and src.mag_029 > 0, an.billing_cost_per_service, max(src.mag_029)) as mag_029,
    if(src.super_ind = 1 and src.mag_030 > 0, an.billing_cost_per_service, max(src.mag_030)) as mag_030,
    if(src.super_ind = 1 and src.mag_031 > 0, an.billing_cost_per_service, max(src.mag_031)) as mag_031,
    if(src.super_ind = 1 and src.mag_032 > 0, an.billing_cost_per_service, max(src.mag_032)) as mag_032,
    if(src.super_ind = 1 and src.mag_033 > 0, an.billing_cost_per_service, max(src.mag_033)) as mag_033,
    if(src.super_ind = 1 and src.mag_034 > 0, an.billing_cost_per_service, max(src.mag_034)) as mag_034,
    if(src.super_ind = 1 and src.mag_035 > 0, an.billing_cost_per_service, max(src.mag_035)) as mag_035,
    if(src.super_ind = 1 and src.mag_037 > 0, an.billing_cost_per_service, max(src.mag_037)) as mag_037
  from
    sup_source src
  left join
    anee_service an
